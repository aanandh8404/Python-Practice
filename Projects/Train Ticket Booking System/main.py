...
class TrainBookingSystem:
    ...
    def add_trains(self):
        print("\n---------Add New Train----------")
        id = int(input("Enter the train no :"))
        name = input("Enter the train name :")
        source = input("Enter the Source :")
        Destination = input("Enter the Destination :")
        seats = int(input("Enter the number of seats :"))
        fair = int(input("Enter the ticket price :"))

        try:
            sql = "insert into trains (id,train_name,source,destination,seats,fair) values (%s,%s,%s,%s,%s,%s)"
            val = (id,name,source,Destination,seats,fair)
    ...
    def view_trains(self):
        ...
        res = cursor.fetchall()
        for i in res:
            print(f"Train no : {i[0]} | Name : {i[1]} | Route : {i[2]} -> {i[3]}  | Seats Available : {i[4]} | fare : {i[5]}")
